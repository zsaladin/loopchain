...
class TestEventSystem(unittest.TestCase):
    ...
    def test_subscriber_register(self):
        ...
        self.assertEqual(len(publisher.subscribers), 4)
        self.assertEqual(len(publisher.subscribers["a"]), 4)
        self.assertEqual(len(publisher.subscribers["b"]), 3)
        self.assertEqual(len(publisher.subscribers["c"]), 1)

        return publisher

    def test_subscriber_unregister(self):
        publisher = self.get_default_subscribers()
        publisher.unregister("a", self.__callback_for_test_order_10)
        self.assertEqual(len(publisher.subscribers["a"]), 3)
        self.assertEqual(len(publisher.subscribers["b"]), 3)
    ...
    def get_default_subscribers(self):
        ...
        publisher.register(subscriber_1)
        publisher.register(subscriber_2)
        publisher.register(subscriber_3)
        publisher.register(subscriber_4)
