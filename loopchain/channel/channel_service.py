...
class ChannelService:
    def __init__(self, channel_name, amqp_target, amqp_key):
        ...
        self.__websocket: WebSocketClient = None
    ...
    async def __subscribe_call_from_citizen(self, rs_rest_stub):
        ...
        try:
            ...
            logging.error(f"__subscribe_call error, caused by : {e}\n"
                          f"This target({ChannelProperty().radio_station_target}) may not support websocket yet.\n")
            await self.__subscribe_call_by_rest_stub(rs_rest_stub)

    async def __subscribe_call_by_websocket(self):
        self.__websocket.block_height = self.block_manager.get_blockchain().block_height
        await self.__websocket.subscribe(self.__websocket.target_uri)
